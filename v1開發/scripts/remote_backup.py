...
def main():
    ...
    try:
        ...
        restore_db = "foodiemo_v1_restore_" + stamp
        ...
        exists = sql("SELECT datname FROM pg_database WHERE datname='foodiemo_v1_test';")
        if exists.strip():
            print("Test database already exists; unchanged.")
            return
        test_password = secrets.token_urlsafe(32)
        sql("CREATE ROLE foodiemo_v1_test_owner LOGIN PASSWORD '" + test_password + "' NOSUPERUSER NOCREATEDB NOCREATEROLE;")
        sql("CREATE DATABASE foodiemo_v1_test OWNER foodiemo_v1_test_owner TEMPLATE template0;")
        sql("REVOKE ALL ON DATABASE foodiemo_v1_test FROM PUBLIC; COMMENT ON DATABASE foodiemo_v1_test IS 'foodiemo-v1-isolated';")
        sql("REVOKE CREATE ON SCHEMA public FROM PUBLIC; GRANT ALL ON SCHEMA public TO foodiemo_v1_test_owner;", "foodiemo_v1_test")
        test_env = (f"APP_ENV=test\nDB_HOST={os.environ['DB_HOST']}\nDB_PORT={os.environ['DB_PORT']}\n"
            f"DB_NAME=foodiemo_v1_test\nDB_USER=foodiemo_v1_test_owner\nDB_PASSWORD={test_password}\n"
            f"TEST_EXPECTED_HOST={os.environ['DB_HOST']}\nTEST_EXPECTED_PORT={os.environ['DB_PORT']}\n"
            "TEST_ALLOW_WRITE=foodiemo-v1-isolated\n")
