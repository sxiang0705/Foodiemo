...
assert.equal(await p.locator('#dragOverlay').evaluate(el=>getComputedStyle(el).pointerEvents),'none');
await pencil.tap();await p.waitForURL('**/edit.html?from=home');
const editor=p.frameLocator('#featureFrame');
assert.equal(await editor.locator('#shareBtn').innerText(),'Finish');
assert.equal(await editor.locator('#choosePhotosBtn').isVisible(),true);
const chooserEvent=p.waitForEvent('filechooser');await editor.locator('#choosePhotosBtn').tap();
const chooser=await chooserEvent;
assert.equal(chooser.isMultiple(),true);
await chooser.setFiles(require('path').resolve(__dirname,'../frontend/IMG_1940.jpg'));
await editor.locator('#feedGrid .grid-item img').waitFor();
const stable=await editor.locator('body').evaluate(()=>{const before=createdUrls.size;handleGridClick(0);handleGridClick(0);return createdUrls.size===before;});
assert.equal(stable,true);console.log('PASS '+width+'px album chooser, thumbnail and reused preview');
p.once('dialog',d=>d.accept());await editor.locator('#backBtn').tap();await p.waitForURL('**/index.html');await p.waitForFunction(()=>document.getElementById('featureOverlay').style.display==='none');console.log('PASS '+width+'px pencil tap, embedded editor and Back');
await c.close();
