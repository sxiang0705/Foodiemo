"""Apply the reviewed 0004 friend requests migration to project_db after a fresh verified backup."""
...
def main():
    ...
    try:
        ...
        if version=="0004_friend_requests":print("0004 already applied; no changes");return
        if version!="0003_record_likes":raise RuntimeError("Expected 0003_record_likes before 0004")
        ...
        print("Verified: project_db is at 0003, matching the fresh backup schema, checksum, and row counts",flush=True)
        if not args.apply:return
        spec=importlib.util.spec_from_file_location("migration",ROOT/"migrations/versions/0004_friend_requests.py");module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
        role='"'+os.environ["DB_USER"].replace('"','""')+'"'
        query="BEGIN; SET LOCAL lock_timeout='5s'; SET LOCAL statement_timeout='30s';\n"+module.DDL
        query+="\nGRANT SELECT,INSERT,UPDATE ON public.friend_requests TO "+role+"; GRANT USAGE,SELECT ON SEQUENCE public.friend_requests_request_id_seq TO "+role+";"
        query+="\nUPDATE public.alembic_version SET version_num='0004_friend_requests' WHERE version_num='0003_record_likes'; COMMIT;"
        sql(query)
        if sql("SELECT version_num FROM public.alembic_version;")!="0004_friend_requests":raise RuntimeError("Post-check revision failed")
        if sql("SELECT to_regclass('public.friend_requests');") not in ("friend_requests","public.friend_requests"):raise RuntimeError("Post-check table failed")
        print("Applied 0004 transactionally; friend_requests table and sequence grants verified",flush=True)
    finally:ssh.close()
