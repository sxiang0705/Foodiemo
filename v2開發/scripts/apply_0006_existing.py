"""Safely apply the reviewed username migration to project_db after a verified backup."""
...
def main():
    ...
    try:
        ...
        if version=="0006_usernames":print("0006 already applied; no changes");return
        if version!="0005_chat_messages":raise RuntimeError("Expected 0005_chat_messages before 0006")
        ...
        print("Verified: project_db is at 0005, matching the fresh backup schema, checksum, and row counts",flush=True)
        if not args.apply:return
        spec=importlib.util.spec_from_file_location("migration",ROOT/"migrations/versions/0006_usernames.py");module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
        ...
        query+="\nUPDATE public.alembic_version SET version_num='0006_usernames' WHERE version_num='0005_chat_messages'; COMMIT;"
        sql(query)
        if sql("SELECT version_num FROM public.alembic_version;")!="0006_usernames":raise RuntimeError("Post-check revision failed")
        if sql("SELECT is_nullable FROM information_schema.columns WHERE table_schema='public' AND table_name='users' AND column_name='username';")!="NO":raise RuntimeError("Post-check username column failed")
        if sql("SELECT to_regclass('public.users_username_lower_uq');") not in ("users_username_lower_uq","public.users_username_lower_uq"):raise RuntimeError("Post-check unique index failed")
        if sql("SELECT count(*) FROM public.users WHERE username IS NULL;")!="0":raise RuntimeError("Post-check null usernames found")
        print("Applied 0006 transactionally; username backfill, not-null constraint, and unique index verified",flush=True)
    finally:ssh.close()
