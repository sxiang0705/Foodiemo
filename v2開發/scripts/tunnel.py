...
def main():
    ...
    def connect():
        ...
        try:
            ...
            ssh.close()
            raise
    reconnect(connect,lambda ssh:forward(ssh,int(os.environ["DB_PORT"])))

def reconnect(connect,serve,sleep=time.sleep):
    delay=2
    while True:
        ssh=None
        try:
            ...
            delay=2
            serve(ssh)
            raise EOFError("SSH transport disconnected")
        except (paramiko.AuthenticationException,paramiko.BadHostKeyException):
            raise  # Credentials and host-key mismatches require attention, never bypass verification.
        except (OSError,EOFError,paramiko.SSHException) as exc:
            print("Tunnel disconnected; reconnecting: "+type(exc).__name__,flush=True)
        finally:
            if ssh:ssh.close()
        sleep(delay)
        delay=min(delay*2,30)

def forward(ssh,port):
    ...
    with Forwarder(("127.0.0.1",port),Handler) as server:
        server.timeout=1
